/*
 * MMSE_SIC_ordering_fast.h
 *
 *  Created on: May 10, 2016
 *      Author: tianpei.chen@mail.mcgill.ca
 */
#include "linkedList.h"
#ifndef MMSE_SIC_ORDERING_FAST_H_
#define MMSE_SIC_ORDERING_FAST_H_
void MMSE_SIC_ordering_fast(gsl_vector_complex *preceived, gsl_matrix_complex *pH,
		 gsl_matrix_complex *WInv, double snr, double pav, int M, gsl_vector_complex *psymOut);
void MMSE_SIC_ordering_fast(gsl_vector_complex *preceived, gsl_matrix_complex *pH,
		 gsl_matrix_complex *WInv, double snr,double pav, int M, gsl_vector_complex *psymOut){
   int Nr = pH->size1;
   int Nt = pH->size2;
   ...
   *signum = 1;
   ...
   for (count = 0; count < Nt; count++){
	   ...
	   if(count == 0){
         gsl_matrix_complex_memcpy(G_preInv, WInv);
	   }else{
...
#endif /* MMSE_SIC_ORDERING_FAST_H_ */
